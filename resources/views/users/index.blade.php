...
<div x-data="{ addModal: false, editModal: false, deleteModal: false, editData: {}, deleteUrl: '', deleteName: '' }">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <!-- Search -->
        <form method="GET" action="{{ route('user.index') }}" class="w-full sm:w-1/3 relative">
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center flex justify-center space-x-2">
                            ...
                            <!-- Delete -->
                            @if($item->id !== auth()->id())
                            <button @click="deleteUrl = '{{ route('user.destroy', $item->id) }}'; deleteName = '{{ $item->name }}'; deleteModal = true" class="text-rose-500 hover:text-rose-700 hover:bg-rose-50 p-1.5 rounded transition">
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div x-show="editModal" x-transition.scale.origin.bottom class="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden transform transition-all">
            ...
            <form :action="'{{ url('user') }}/' + editData.id" method="POST" class="p-6">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
